...
#    jq 1.6 (https://stedolan.github.io/jq/), and yq 4.x (https://github.com/mikefarah/yq)
# 2) Create a temporary work folder to use to generate your template
# 3) Download and copy `toolchain-to-template-yq4.sh` to your work folder
...
#
# RUN THE SCRIPT
# In a shell run the following: `./toolchain-to-template-yq4.sh https://your-toolchain-url`
#
...
function download_classic_pipeline() {
  ...
  yq eval -o=json ${SOURCE_PIPELINE_ID}.yaml > ${SOURCE_PIPELINE_ID}.json
  ...
  yq -o yaml -P $TARGET_PIPELINE_ID.json > $TARGET_PIPELINE_ID.yml
...
function download_tekton_pipeline() {
  ...
  yq eval -P $TARGET_PIPELINE_ID.json > $TARGET_PIPELINE_ID.yml
...
WRONG_YQ=$( yq --version 2>&1 | grep "yq (https://github.com/mikefarah/yq/) version [4]\." )
if [ -z "${WRONG_YQ}" ]; then
  echo "Unexpected prereq 'yq --version' is not 4.x"
  exit 1
fi
...
do
    ...
    # orion
    SERVICE_ID=$(yq ".services[${i}].service_id" "${OLD_TOOLCHAIN_JSON_FILE}")
    ...
    SERVICE_INSTANCE_ID=$(yq ".services[${i}].instance_id" "${OLD_TOOLCHAIN_JSON_FILE}")

    # 'services[0].toolchain_binding.name' is like:
    # webide
    BINDING_NAME=$(yq ".services[${i}].toolchain_binding.name" "${OLD_TOOLCHAIN_JSON_FILE}")
    ...
    SERVICE_NAME=$(yq ".services[${i}].parameters.name" "${OLD_TOOLCHAIN_JSON_FILE}")
    if [ -z "${SERVICE_NAME}" ] || [ 'null' = "$SERVICE_NAME" ] ; then
      SERVICE_NAME="$BINDING_NAME"
    fi
    REPO_URL=$(yq ".services[${i}].parameters.repo_url" "${OLD_TOOLCHAIN_JSON_FILE}")
...
do
    ...
    SERVICE_PARAMETERS=$(yq ".services[${i}].parameters" "${OLD_TOOLCHAIN_JSON_FILE}")
    ...
    if [ 'pipeline' = "${SERVICE_ID}"  ] ; then
        ...
        if [ "tekton" == "$PIPELINE_TYPE" ]; then
          ...
          SERVICE_DASHBOARD_URL=$(yq ".services[${i}].dashboard_url" "${OLD_TOOLCHAIN_JSON_FILE}")
          SERVICE_REGION_ID=$(yq ".services[${i}].region_id" "${OLD_TOOLCHAIN_JSON_FILE}")
